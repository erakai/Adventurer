...
std::unordered_map<KeyEventType, std::vector<std::function<void()>>> adv::input::key_callbacks;
std::vector<std::function<void(KeyEventType k)>> adv::input::general_key_callbacks;
...
{
  for (auto &f: key_callbacks[e]) f();
  for (auto &f: general_key_callbacks) f(e);
}

void adv::input::run_before_input_hooks()
{
  for (auto &f: before_input_callbacks) f();
}
