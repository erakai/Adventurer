...
void QuadTree::split()
{
  ...
  nodes[0] = std::make_shared<QuadTree>(
      level + 1, Rect(x, y, x + new_width, y + new_height)); // TL
  nodes[1] = std::make_shared<QuadTree>(
      level + 1,
      Rect(x + new_width, y, x + new_width + new_width, y + new_height)); // TR
  nodes[2] = std::make_shared<QuadTree>(
      level + 1, Rect(x, y + new_height, x + new_width,
                      y + new_height + new_height)); // BL
  nodes[3] = std::make_shared<QuadTree>(
      level + 1, Rect(x + new_width, y + new_height, x + new_width + new_width,
                      y + new_height + new_height)); // BR
...
QuadTree::retrieve_nearby(const Positioned obj)
{
...
void QuadTree::retrieve(std::vector<std::shared_ptr<Positioned>> &return_objs,
                        Positioned obj)
{
  int index = get_index(obj);
